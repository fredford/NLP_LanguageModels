...
class Laplace:
	'''
	This class creates an object that runs a Laplace model
	'''
	...
	def process_train(self, dataFile, filename):
		...
		# Read the text file into a single string while stripping the newline character
		lines = ""
	...
	def probability(self, token, filename):
		...
		if self.counter[filename][token[0]] == 0:
			return 0
	
		# Compute the probability using the count, number of tokens, and size of vocabulary
		return (self.counter[filename][token[0]] + 1) / ((self.length[filename])+len(self.vocabulary[filename]))
